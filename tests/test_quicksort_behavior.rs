...
fn test_quicksort_resumes_with_small_budgets() {
    ...
    // Verify completion
    assert!(quicksort.is_complete(), "Failed to complete after {} steps", step_count);
    
    // Verify correctness
    let mut expected = data;
    expected.sort();
    assert_eq!(quicksort.get_array(), &expected[..], "Array not correctly sorted");
}
...
fn test_quicksort_budget_performance_tradeoff() {
    ...
    for budget in budgets {
        ...
        expected.sort();
        assert_eq!(quicksort.get_array(), &expected[..],
            "Incorrect result with budget {}", budget);
    }
...
fn test_quicksort_telemetry_quality() {
    ...
    assert!(!final_telemetry.status_text.is_empty(),
        "No status text provided");
    
    // Progress should be monotonic
    for window in telemetry_history.windows(2) {
        assert!(window[1].progress_hint >= window[0].progress_hint,
            "Progress decreased: {} -> {}", 
            window[0].progress_hint, window[1].progress_hint);
    }
}