...
impl HighlightSystem {
    ...
    pub fn new() -> Self {
        ...
        color_map.insert("compare".to_string(), (HighlightColor::Blue, HighlightPriority::Medium));
        color_map.insert("swap".to_string(), (HighlightColor::Red, HighlightPriority::High));
        color_map.insert("move".to_string(), (HighlightColor::Red, HighlightPriority::High));
        color_map.insert("pivot".to_string(), (HighlightColor::Yellow, HighlightPriority::Critical));
        color_map.insert("sorted".to_string(), (HighlightColor::Green, HighlightPriority::Low));
        color_map.insert("cursor".to_string(), (HighlightColor::Purple, HighlightPriority::Medium));
    ...
    pub fn highlight_comparison(&mut self, indices: &[usize]) {
        for &index in indices {
            ...
            self.highlights.entry(index).or_insert_with(Vec::new).push(highlight);
        }
    }
    ...
    pub fn highlight_operation(&mut self, operation_type: &str, indices: &[usize]) {
        let (color, priority) = self.color_map.get(operation_type)
            .copied()
    ...
    pub fn get_highlights_for_indices(&self, indices: &[usize]) -> HashMap<usize, Option<&Highlight>> {
        indices.iter().map(|&index| {
            (index, self.get_highlight(index))
        }).collect()
    }
    ...
    pub fn set_color_mapping(&mut self, operation_type: &str, color: HighlightColor, priority: HighlightPriority) {
        self.color_map.insert(operation_type.to_string(), (color, priority));
    }

    /// Get color mapping for operation type
    pub fn get_color_mapping(&self, operation_type: &str) -> Option<(HighlightColor, HighlightPriority)> {
    ...
    pub fn from_telemetry_highlights(&mut self, telemetry_highlights: &[usize], operation_type: &str) {
...
impl TelemetryHighlightIntegration {
    ...
    pub fn update_from_telemetry(&mut self, telemetry: &sorting_race::models::traits::Telemetry) {
        ...
        if !telemetry.highlights.is_empty() {
            ...
            self.highlight_system.from_telemetry_highlights(&telemetry.highlights, operation_type);
        }
        ...
        for &cursor in &telemetry.markers.cursors {
            self.highlight_system.highlight_operation("cursor", &[cursor]);
        }
...
mod tests {
    use super::*;
    use sorting_race::models::traits::{Telemetry, Markers};
