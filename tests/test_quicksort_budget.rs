...
fn test_quicksort_progress_with_k16_n50() {
    ...
    while !quicksort.is_complete() && step_count < 200 {
        ...
        assert!(result.comparisons_used <= BUDGET_K, 
            "Step {} exceeded budget: used {} but limit was {}", 
            step_count, result.comparisons_used, BUDGET_K);
    ...
    if !quicksort.is_complete() {
        panic!("Incremental partitioning with k=16 not working: failed to complete after {} steps", step_count);
    }
    ...
    println!("Completed in {} steps with {} total comparisons", step_count, total_comparisons);
...
fn test_completion_within_reasonable_steps() {
    ...
    let test_cases = vec![
        ...
        ("Alternating", (0..ARRAY_SIZE_N as i32).map(|i| if i % 2 == 0 { i } else { -i }).collect()),
    ];
    ...
    for (name, data) in test_cases {
        ...
        while !quicksort.is_complete() && steps < max_reasonable_steps * 2 {
            ...
            assert!(result.comparisons_used <= BUDGET_K,
                ...
                name, steps, result.comparisons_used, BUDGET_K);
        }
        
        // Should complete within reasonable bounds
        if !quicksort.is_complete() {
            panic!("Case '{}' did not complete within {} steps", name, max_reasonable_steps * 2);
        }
        
        // Should complete within expected step count
        assert!(steps <= max_reasonable_steps,
            "Case '{}' took {} steps but expected <= {}", 
            name, steps, max_reasonable_steps);
        
        // Verify correctness
        let final_array = quicksort.get_array();
        let mut expected = data.clone();
        expected.sort();
        assert_eq!(final_array, expected, 
            "Case '{}' did not produce sorted array", name);
...
fn test_sorting_correctness_with_k16() {
    ...
    let test_arrays = vec![
        ...
        // Random-ish pattern
        vec![25, 12, 45, 7, 33, 19, 41, 8, 36, 14, 47, 3, 29, 18, 42, 
             6, 31, 16, 44, 9, 37, 13, 48, 2, 26, 17, 43, 5, 32, 11, 
             46, 4, 28, 20, 40, 10, 35, 15, 49, 1, 27, 21, 39, 22, 34, 
             23, 38, 24, 50, 30],
    ];
    ...
    for (i, mut data) in test_arrays.into_iter().enumerate() {
        ...
        // Must complete
        if !quicksort.is_complete() {
            panic!("Configuration {} failed to complete after {} steps", i, steps);
        }
        ...
        assert_eq!(final_telemetry.progress_hint, 1.0,
            "Progress should be 1.0 when complete, got {}", final_telemetry.progress_hint);
        
        assert!(final_telemetry.total_comparisons > 0,
            "Should have made some comparisons for non-trivial array");
        
        println!("Configuration {} passed: {} steps, {} comparisons", 
...
fn test_resource_usage_telemetry_k16() {
    ...
    let total_comparisons: usize = step_results.iter()
        .map(|r| r.comparisons_used)
        .sum();
    
    let total_moves: usize = step_results.iter()
        .map(|r| r.moves_made)
        .sum();
    ...
    assert!(total_comparisons >= expected_min_comparisons,
        "Too few comparisons: {} < {} for n={}", 
        total_comparisons, expected_min_comparisons, n);
    
    assert!(total_comparisons <= expected_max_comparisons,
        "Too many comparisons: {} > {} for n={}", 
        total_comparisons, expected_max_comparisons, n);
    ...
    assert!(max_memory_seen <= reasonable_memory_bound,
        "Memory usage too high: {} > {} bytes", 
        max_memory_seen, reasonable_memory_bound);
    
    // Telemetry should show meaningful progress
    let progress_values: Vec<f32> = telemetries.iter()
        .map(|t| t.progress_hint)
        .collect();
    ...
    assert!(progress_decreased_count <= progress_values.len() / 10,
        "Progress decreased too often: {} times out of {} steps", 
        progress_decreased_count, progress_values.len());
    
    println!("Resource usage - Steps: {}, Comparisons: {}, Moves: {}, Max Memory: {} bytes",
             step_results.len(), total_comparisons, total_moves, max_memory_seen);
