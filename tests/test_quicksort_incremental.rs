...
fn test_quicksort_with_limited_budget() {
    ...
    let data = vec![64, 34, 25, 12, 22, 11, 90, 5, 77, 30, 40, 60, 35, 65, 15, 85];
...
fn test_completion_guarantee_with_minimal_budget() {
    ...
    while !quicksort.is_complete() && total_steps < max_expected_steps {
        ...
        if total_steps > max_expected_steps / 2 {
            assert!(result.comparisons_used > 0 || quicksort.is_complete(),
    ...
    let mut expected = data.clone();
    expected.sort();
    assert_eq!(final_array, expected, "Array not properly sorted with minimal budget");
...
fn test_budget_allocation_across_partitions() {
    ...
    assert!(total_comparisons > 0, "No comparisons made across all steps");
    assert!(step_results.len() > 1, "Should require multiple steps with budget {}", budget);
    
    // At least some steps should use the full budget (when work is available)
    let full_budget_steps = step_results.iter()
        .filter(|r| r.comparisons_used == budget && r.continued)
        .count();
    
    assert!(full_budget_steps > 0, 
...
fn test_incremental_partitioning_pivot_strategies() {
    ...
    let pivot_changes = telemetry_history.iter()
        .map(|t| t.markers.pivot)
        .collect::<Vec<_>>();
    
    // Should have multiple different pivots during execution
    let unique_pivots: std::collections::HashSet<_> = pivot_changes
        .into_iter()
        .filter_map(|p| p)
        .collect();
    
    assert!(unique_pivots.len() > 1, 
